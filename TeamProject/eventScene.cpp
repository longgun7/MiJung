...
HRESULT eventScene::init(void)
{
	...
	_map->init(EVENT);
	_pm->getPlayer()->setX(150);
	_pm->getPlayer()->setY(70);
	
	_pm->getPlayer2()->setMove(S_DOWN);
	_pm->getPlayer()->setSlopeNum(4);
...
void eventScene::update(void)
{
	...
	if (KEYMANAGER->isOnceKeyDown(VK_RETURN) && _pm->getPlayer()->getMove() == RIGHT4 || 
		KEYMANAGER->isOnceKeyDown(VK_RETURN) && _pm->getPlayer()->getMove() == LEFT4 || 
		KEYMANAGER->isOnceKeyDown(VK_RETURN) && _pm->getPlayer()->getMove() == WOODRIGHT4 || 
		KEYMANAGER->isOnceKeyDown(VK_RETURN) && _pm->getPlayer()->getMove() == WOODLEFT4  )
	{
		init();
