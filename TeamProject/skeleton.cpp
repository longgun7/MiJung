...
HRESULT skeleton::init()
{
	...
	_direction = STAND;				// 기본 상태는 가만히 있는 상태
	_moveCount2 = 0;					// 일정 시간마다 움직임을 바꾸기 위한 카운트 0으로 초기화
	_speed = 1.0f;						// 이동속도는 1.0f
	_index = _currentFrameY = 0;		// 프레임 이미지 Y와 프레임 감을 인덱스를 0으로 초기화
	_count = 0;						// 프레임 감을 카운트
...
void skeleton::move()
{
	...
	{
		_direction = LEFT_MOVE;											// 상태를 왼쪽 이동
	...
	{
		_direction = RIGHT_MOVE;										// 상태는 오른쪽 이동
	...
	{
		_direction = DOWN_MOVE;											// 상태는 아래로 이동
		_currentFrameY = 0;												// 현재 프레임 Y는 0

		_y += _speed;													// y좌표를 speed만큼 더해준다
	}
	if (_direction == STAND)											// 상태가 가만히 있는 상태라면
	{
	...
	if (_count % 10 == 0)										// _count 나누기 10이 0이라면
	{
		if (_direction == STAND)								// 상태가 가만히 있는 상태라면
		{
			if (_index > 1) _index = 0;							// _index가 1보다 크면 _index를 0으로 초기화한다

			_img->setFrameX(_index);							// _img의 setFrameX에 _index 값을 넣어준다
			_index++;											// _index를 ++해준다
		}
		else													// if 조건이 아니라면
		{
			if (_index >= _img->getMaxFrameX()) _index = 0;		// _index가 _img의 getMaxFrameX보다 크거나 같을 경우 _index를 0으로 초기화한다
